...
@section('content')
    <div class="bg-white dark:bg-gray-800 shadow-md rounded-2xl p-6">
        ...
        {{-- Filtros e Busca --}}
        <form method="GET" action="{{ route('patients.index') }}" class="mb-6 flex flex-col md:flex-row gap-4">
            <div class="relative flex-1">
                ...
                <input
                    ...
                    class="block w-full pl-10 pr-3 py-2 border border-gray-300 dark:border-gray-600 rounded-xl leading-5 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-200 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-primary-500 sm:text-sm transition duration-150 ease-in-out"
                >
            </div>
            <div class="flex gap-2">
                <button type="submit" class="inline-flex items-center justify-center px-4 py-2 bg-primary-600 border border-transparent rounded-xl font-semibold text-xs text-white uppercase tracking-widest hover:bg-primary-700 active:bg-primary-900 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 transition ease-in-out duration-150">
                    Buscar
                </button>
                @if(request()->filled('search'))
                    <a href="{{ route('patients.index') }}" class="inline-flex items-center justify-center px-4 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-xl font-semibold text-xs text-gray-700 dark:text-gray-200 uppercase tracking-widest hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 transition ease-in-out duration-150">
                        Limpar
                    </a>
                @endif
            </div>
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-10 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    @if(request()->filled('search'))
                                        <a href="{{ route('patients.index') }}" class="mt-4 text-primary-600 hover:underline font-bold">Limpar busca</a>
                                    @else
