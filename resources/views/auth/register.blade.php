<x-guest-layout>
    <div class="text-center mb-6">
        <h2 class="text-xl font-bold text-gray-800 dark:text-white">Começar teste grátis</h2>
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Informe seu e-mail para receber o link de cadastro</p>
    </div>
    ...
    <form method="POST" action="{{ route('register') }}" class="space-y-4">
        ...
        <div class="text-center text-xs text-gray-500 dark:text-gray-400">
            {{ __('Já tem uma conta?') }}
            <a href="{{ route('login') }}" class="text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300 font-medium transition-colors duration-200">
                {{ __('Faça login') }}
            </a>
        </div>
    </form>
