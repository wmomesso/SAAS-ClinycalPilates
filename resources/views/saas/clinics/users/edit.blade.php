...
@section('content')
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
        <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
            ...
            <form action="{{ route('clinic-users.update', $user->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name"
                               ...
                               placeholder="Ex: João da Silva" value="{{ $user->name ?? old('name') }}" required>
                        @error('name')
                    ...
                    <div>
                        ...
                        <input type="email" name="email" id="email"
                               class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                               placeholder="usuario@email.com" value="{{ $user->email ?? old('email') }}" required>
                        @error('email')
                        <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div id="calendar_color_container" class="hidden">
                        ...
                        <div class="flex items-center space-x-2">
                            <input type="color" name="calendar_color" id="calendar_color"
                                   ...
                                   value="{{ old('calendar_color', $user->calendar_color ?? '#3b82f6') }}">
                    ...
                    <div>
                        <label for="is_active" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Status de acesso</label>
                        <select id="is_active" name="is_active" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white" required>
                            <option value="1" @selected(old('is_active', $user->is_active) == 1)>Ativo</option>
                            <option value="0" @selected(old('is_active', $user->is_active) == 0)>Inativo</option>
                        </select>
                        @error('is_active')
                        <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                        @enderror
                ...
                <div class="mt-6 flex justify-end">
                    <button type="submit"
                            class="text-white bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-200 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-primary-500 dark:hover:bg-primary-600 dark:focus:ring-primary-800">
                        Atualizar Usuário
                    </button>
                </div>
            </form>
